...
For example, the following command will look up, with a TCP connect scan, all open ports on IP `192.168.1.100` and will try to determine what services are bound to them (only _essential_ switches are shown – nmap features a broad set of options, whose discussion is out of scope):
...
![OWASP Whois Info](https://owasp.org/www-project-web-security-testing-guide/v42/4-Web_Application_Security_Testing/01-Information_Gathering/images/Owasp-Info.jpg)  
_Figure 4.1.4-1: OWASP Whois Info_
...
Googling techniques are explained in [Testing: Spiders, Robots, and Crawlers](https://owasp.org/www-project-web-security-testing-guide/v42/4-Web_Application_Security_Testing/01-Information_Gathering/01-Conduct_Search_Engine_Discovery_Reconnaissance_for_Information_Leakage).
